...
%   These operations may be implemented within the List ADT but as Gamma et
%   al.[1] wonderfully puts it, the key idea in this pattern is to take the
%   responsibility for access and traversal out of the list object and put
%   it into the iterator object - given the iterator describes behaviour
%   whilst the list describes a collection.
%
%   Implementation:  The Iterator abstract class is used purely to specify
%   the requirements of its concrete implementation. Since MATLAB and
%   MATLAB OOP is loosely typed, the appealing software approach of
%   polymorphic iteration is not supported should the user want to
%   implement different realizations of the Iterator. Further, this
%   abstraction could be implemented as an external/active or
%   internal/passive iterator - 
...
%      Boston: Addison-Wesley, 1995.
    ...
    methods (Abstract) % Public Access
        ...
        %             corresponding collection - an empty array is returned
        %             for traversals that have been exceeded - a single
        %             element is returned if only a single list is
        %             traversed
        % Preconditions:
        ...
        next = hasNext(obj);
        % Check if there is another element in the traversal of the
        % collection.
        % Input:
        %   obj     = array of instances of complete implementation of this
        %             abstraction
        % Output:
        %   next    = array of boolean values 'true' if elements remaining
        %             in each corresponding traversal
        % Preconditions:
        %   <none>
        % Postconditions:
        %   <none>
        ...
        reset(obj);
        % Overloaded. Reset the Iterator to first element in the collection
        % Input:
        %   obj     = array of instances of concrete implementation of this
        %             abstraction
        % Output:
