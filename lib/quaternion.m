...
% l = (q1 == q2) | (q1 == -q2)
% optional argument tol (default = eps(16)) sets tolerance for difference
% from exact equality
        if ~isa( q1, 'quaternion' )
            q1  = quaternion( real(q1), imag(q1), 0, 0 );
        end
...
% series of quaternions. The angular velocity and vector are computed by:
%  qe    = [q.e];
%  pp    = spline( t, qe ); % piecewise polynomial
%  pd    = ppderiv( pp );   % piecewise polynomial derivative
%  qv    = ppval( pd, t );
%  omega = 2 * sqrt( sum( qv.^2, 1 ));
%  axis  = repmat( sgn(qe(1,:)), [3,1] ) .* unitvector( qv(2:4,:), 1 );
% Inputs:
...
% Code from 2016
        ...
        pd            = ppderiv( pp );
        qv            = ppval( pd, t );
        qv            = qv - bsxfun( @times, sum( qe .* qv, 1 ), qe );
        omega         = 2 * sqrt( sum( qv.^2, 1 ));
        omega         = reshape( omega, siq );
        axis          = unitvector( qv(2:4,:), 1 );
        axis          = bsxfun( @times, sgn( qe(1,:) ), axis );
        if dim > 1
...
%  quaternions in array q
        ...
        or  = zeros(1,3);
        ax  = eye(3);
        alx = q.RotationMatrix;
        newplot;
        old = get( 0, 'defaultAxesColorOrder' );
        set( 0, 'defaultAxesColorOrder', [0,0,1; 0,0.5,0; 1,0,0] );
        for iel = 1 : nel
            plot3( [or; ax(1,:)], [or ; ax(2,:)], [or; ax(3,:)], ':', 'LineWidth', 2 );
            hold on
            set( gca, 'Xlim', [-1, 1], 'Ylim', [-1, 1], 'Zlim', [-1, 1] );
            xlabel( 'x' );
            ylabel( 'y' );
            zlabel( 'z' );
            plot3( [or; alx(1,:,iel) ], [or ; alx(2,:,iel)], [or; alx(3,:,iel)], '-', 'LineWidth', 2 );
            plot3( squeeze(alx(1,:,1:iel)).', squeeze(alx(2,:,1:iel)).', squeeze(alx(3,:,1:iel)).', '*' );
            grid on
            if interval
                pause( interval );
            end
        ...
        set( 0, 'defaultAxesColorOrder', old ); 
    end % PlotRotation
...
function pd = ppderiv( pp, order )
% function pd = ppderiv( pp, order )
% Piecewise polynomial derivative
% Inputs:
%  pp		matlab piecewise polynomial struct from spline or mkpp
%  order [OPTIONAL] derivative order >= 1, default = 1
%           if order > 1, ppderiv calls itself recursively
% Output:
%  pd		piecewise polynomial derivative struct
pd = pp;
for ip = 1 : numel( pd )
    pd(ip).order = max( 0, pd(ip).order - 1 );
    if pd(ip).order == 0
        pd(ip).coefs = zeros( size( pp(ip).coefs, 1 ), 1 );
    else
        pd(ip).coefs = bsxfun( @times, ...
                       pd(ip).coefs(:,1:pd(ip).order), pd(ip).order:-1:1 );
    end
end
if (nargin > 1) && any( order > 1 )
    pd = ppderiv( pd, order-1 );
end
end % ppderiv
