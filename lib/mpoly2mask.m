...
A = varargin{end};
if islogical(A)
    varargin(end) = []; % Use last param as association matrix
elseif isnumeric(A) && ismatrix(A) && size(A,1)==size(A,2) && all(ismember(A(:),[0 1]))
    A = logical(A);
    varargin(end) = [];
else % No association matrix was provided
