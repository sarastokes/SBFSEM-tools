classdef OData < handle
    ...
    properties (Access = private)
        % Neuron structure data, fetched automatically
        vikingData
        % Location data, use getNodes()
        nodeData
        % Link data, use getEdges()
        edgeData
        % Synapse data, use getSynapses(), adds to nodeData and edgeData
        childData = struct()
        % Volume dimensions
        volumeData
    end
    ...
    methods
        ...
        function [viking, nodes, edges, child, volumeScale] = pull(obj)
            ...
            viking = obj.vikingData;
            nodes = obj.fetchNodes();
            edges = obj.fetchEdges();
            child = obj.fetchSynapses();
    ...
    methods (Access = private)
        ...
        function LocLinks = fetchLinkData(obj, ID)
            % FETCHLINKDATA  Returns edges for given ID (default obj.ID)
            if nargin < 2
                ID = obj.ID;
            end
